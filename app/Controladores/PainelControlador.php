...
{
    private const PAGINAS = ['inicio','custos','usuarios','usuario_formulario','indicacao','perfil_inicial'];
    public function __construct(private readonly UsuarioRepositorio $usuarios,private readonly CustoFixoRepositorio $custos,private readonly ResumoFinanceiroRepositorio $resumos,private readonly ServicoPresenca $presenca,private readonly string $diretorioVisoes) {}
    ...
    private function processarFormulario(array $dados,string $idioma): never
    {
        ...
        if($acao==='completar_perfil'){
            ...
            if($completo)$this->resumos->atualizarRenda($id,$this->normalizarValor((string)($dados['renda_mensal']??'0')));
